/**
 * pages/admin/RekapEksporSemuaUPT.jsx
...
export default function RekapEksporSemuaUPT() {
  ...
  async function loadInitialData() {
    ...
    const [{ data: jds }, { data: upts }] = await Promise.all([
      supabase.from('jenis_data').select('*').eq('aktif', true).order('created_at'),
      supabase.from('upt_list').select('*').eq('aktif', true).order('label'),
    ])
    const allJds = jds || []
  ...
  async function handleExportGabungan() {
    ...
    try {
      ...
      // 3. Ambil rekap_nilai
      const { data: rawRekap } = await supabase
        .from('rekap_nilai')
        .select('*')
        .eq('jenis_data_id', selectedJdId)
        .in('period_id', periodIds)
      ...
      ;(rawEntries || []).forEach(e => {
        if (e.data_ekstra && Object.keys(e.data_ekstra).length > 0) {
          kolomTidakDikenal.push({
            UPT: e.upt_key,
            Periode: periodsMap[e.period_id]?.label || '',
            Nama: e.nama || '',
